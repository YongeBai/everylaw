...
const inputSchema = z.object({ nodeId: z.number().int().positive(), direction: z.enum(["keep", "dissolve"]).nullable() });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (input.direction === null) {
      await db.execute(sql`DELETE FROM votes WHERE node_id=${input.nodeId} AND voter_hash=${identity.voterHash}`);
    } else {
    ...
    const [, aggregate] = await Promise.all([
      ...
      db.execute(sql`SELECT COALESCE(a.keep_count,0) keep_count, COALESCE(a.dissolve_count,0) dissolve_count, COALESCE(a.total_count,0) total_count, COALESCE(a.dissolve_ratio,0) dissolve_ratio FROM law_nodes n LEFT JOIN vote_aggregates a ON a.node_id=n.id WHERE n.id=${input.nodeId}`),
    ]);
