...
type Take = { id: number; stance: "keep"|"dissolve"|null; body: string; upvoteCount: number; createdAt: string };
...
export function TakesBoard({ nodeId, initialTakes }: { nodeId: number; initialTakes: Take[] }) {
  ...
  return <section id="takes"><div className="flex items-end justify-between gap-5"><div><p className="eyebrow">Structured public reasoning</p><h2 className="serif text-4xl font-black mt-2">Make the case</h2></div><span className="text-sm text-[#68736d]">No replies. No pile-ons. 280 characters.</span></div>
    <form onSubmit={submit} className="paper-card rounded-2xl p-5 mt-6">
      <p className="text-sm text-[#68736d]">Your case carries your current vote on this law.</p>
      <label className="sr-only" htmlFor="take-body">Your argument</label><textarea data-testid="take-body" id="take-body" required minLength={3} maxLength={280} value={body} onChange={(e) => setBody(e.target.value)} placeholder="Make one concise, useful argument." className="mt-4 w-full min-h-24 resize-y rounded-xl border border-[#b8aa94] p-4 bg-white outline-none focus:ring-2 focus:ring-[#d9a62e]" />
    ...
    {takes.some((take) => take.stance === null) && <div className="mt-8"><h3 className="serif text-2xl font-black text-[#68736d]">Unaligned cases</h3><div className="space-y-3 mt-4">{takes.filter((take) => take.stance === null).map((take) => <article data-testid="take" key={take.id} className="paper-card rounded-xl p-4"><p className="leading-relaxed">{take.body}</p><button data-testid={`upvote-${take.id}`} onClick={() => upvote(take.id)} className="mt-3 text-sm font-black hover:underline">▲ {take.upvoteCount}</button></article>)}</div></div>}
  </section>;
}
