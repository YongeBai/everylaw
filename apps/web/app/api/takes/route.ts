...
const schema = z.object({ nodeId: z.number().int().positive(), stance: z.enum(["keep", "dissolve"]), body: z.string().trim().min(3).max(280), website: z.string().max(0), parentId: z.number().int().positive().optional() });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows = await db.execute(sql`INSERT INTO takes(node_id, voter_hash, stance, body, parent_id) VALUES (${input.nodeId}, ${identity.voterHash}, ${input.stance}::vote_direction, ${input.body}, ${input.parentId ?? null}) RETURNING id, stance, body, upvote_count, downvote_count, parent_id, created_at`);
    await recordInteraction("take", identity);
    const row = rows[0]!;
    return NextResponse.json({ take: { id: Number(row.id), stance: String(row.stance), body: String(row.body), upvoteCount: Number(row.upvote_count), downvoteCount: Number(row.downvote_count), parentId: row.parent_id === null ? null : Number(row.parent_id), createdAt: String(row.created_at) } }, { status: 201 });
  } catch (error) {
