...
Why the work matters: a democracy can only debate, fix, or repeal its laws when citizens actually understand them. Your translation is what makes a reader informed enough to hold an opinion — to notice that a law is outdated, that it isn't doing what it was written to do, or that it quietly matters more than anyone realizes — and to argue about it in public. Write like that outcome depends on this page, because it does.
...
- Never leave a technical term, chemical, or unfamiliar name unexplained. At first mention, give the reader the everyday handle: the common name ("pseudoephedrine — the decongestant in Sudafed"), what kind of thing it is ("a *list I chemical* — an ingredient the government tracks because it can be used to make illegal drugs"), or the scale that makes a number meaningful. If a reader would have to look something up to follow the sentence, the sentence isn't finished.
...
Two kinds of examples earn their place every time:

- **A worked example for every table or formula.** Numbers a reader can't apply are decoration. After a rate table: "So a married couple with $100,000 of taxable income doesn't pay 22% on all of it — the first $77,400 goes through the lower brackets, and only the rest is taxed at 22%."
- **Negative space — what the law does NOT cover.** Boundaries ground understanding: the five-room exception "keeps the law from reaching someone renting out rooms in their own home"; a criminal statute that only applies on federal land; conduct the section leaves to the states. A notable thing a law *doesn't* do is also exactly what readers debate — surface it.
...
- Background on *why* a law exists is the one welcome exception: when a section sounds absurd or arbitrary without its history, one or two sentences of widely-accepted context make it click — framed as background ("generally understood as a response to …", "dates from an era when …"), never invented specifics presented as fact. If you don't actually know the history, say what the text itself suggests and no more.
